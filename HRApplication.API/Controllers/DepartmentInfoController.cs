...
public class DepartmentInfoController : ControllerBase
{
    ...
    [HttpGet]
    [Route("GetTestAPI")]
    public async Task<IActionResult> GetTestAPI(long _departmentId)
    {
        var request = new GetDepartmentInfoDetailsByIdRequest { DepartmentId = _departmentId };
        var response = await _mediator.Send(request);
        return Ok(response);
    ...
    [HttpGet]
    [Route("GetTestAPI2")]
    public async Task<IActionResult> GetTestAPI2(long _departmentId)
    {
        var request = new GetDepartmentInfoDetailsByIdRequest { DepartmentId = _departmentId };
        var response = await _mediator.Send(request);
        return Ok(response);
    ...
    [HttpGet]
    [Route("GetTestAPI3")]
    public async Task<IActionResult> GetTestAPI3(long _departmentId)
    {
        var request = new GetDepartmentInfoDetailsByIdRequest { DepartmentId = _departmentId };
        var response = await _mediator.Send(request);
        return Ok(response);
